...
function scoresAverage(moviesArray) {
	...
	for(let i = 0; i<moviesArray.length; i++){
		if (moviesArray[i] !== undefined && moviesArray[i].score !== undefined && moviesArray[i].score !== ''){
...
function dramaMoviesScore(moviesArray) {
  ...
  dramaMovies.forEach(movie => sum = movie.score+sum)
...
function orderByYear(moviesArray) {
  ...
  moviesArray.forEach((movie) => {
      if(movie.title !== undefined){
          yearOfMovie.push({year: movie.year, title:movie.title});
      }else{
          yearOfMovie.push({year: movie.year});
      }
  })
  yearOfMovie.sort(function (a, b) {
    if (a.year === b.year){
...
function orderAlphabetically(moviesArray) {
  ...
  let alphabeticalArray = [...moviesArray];

  alphabeticalArray.sort(function (a, b) {
    return a.title.localeCompare(b.title);
  }); // sort the entire 250 movies
  let firstTwenty = alphabeticalArray.slice(0, 20); // get the first 20 movies
